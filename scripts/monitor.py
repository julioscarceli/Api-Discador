...
SAIDAS_VALOR = "18"  # Alterado para 18 canais conforme solicitado
...
SELETOR_AGRESSIVIDADE_BOTAO = '#Agressive > div > label > span.toggle' # Reinserido
...
async def restart_campaign(server: str):
    """Fluxo com agressividade ativada e 18 canais."""
    async with async_playwright() as p:
        ...
        try:
            ...
            # Aba Enviar e detecção de mailing
            ...
            # --- BOTÃO DE AGRESSIVIDADE REATIVADO ---
            print(f"[{server}] Ativando agressividade por agente...")
            await page.wait_for_selector(SELETOR_AGRESSIVIDADE_BOTAO)
            await page.click(SELETOR_AGRESSIVIDADE_BOTAO)
            
            # Disparo com 18 canais
            ...
            print(f"[{server}] ✅ RESTART CONCLUÍDO COM 18 CANAIS E AGRESSIVIDADE ATIVA.")
            return True
