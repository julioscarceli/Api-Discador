...
async def finalize_campaign_only(server: str):
    """Finaliza a campanha ativa em SP."""
    async with async_playwright() as p:
        ...
        try:
            print("[SP] Finalizando: Acessando URL Direta...")
            await page.goto(URL_DA_SP, wait_until="commit", timeout=60000)
            ...
            if "da.php" not in page.url:
                print(f"[SP] Desvio detectado ({page.url}). Forçando DA...")
                await page.goto(URL_DA_SP, wait_until="domcontentloaded", timeout=45000)

            print("[SP] Sincronizando aba Enviar...")
            ...
            if await btn_finalizar.count() > 0:
                print("[SP] 🖱️ Disparando Finalizar...")
                ...
                print("[SP] ✅ Campanha FINALIZADA com sucesso.")
            else:
                print("[SP] ℹ️ Campanha já estava parada.")

            return True

        except Exception as e:
            print(f"[SP] ❌ Falha na finalização: {e}")
            return False
        finally:
            await browser.close()

async def restart_campaign(server: str):
    """Fluxo focado em SP sem o clique na agressividade e com 150 canais."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=HEADLESS)
        context = await browser.new_context(ignore_https_errors=True)
        page = await context.new_page()

        fila_name = "DISCADOR_SP"

        try:
            print("[SP] Acessando URL Direta...")
            await page.goto(URL_DA_SP, wait_until="commit", timeout=60000)
            ...
            if "da.php" not in page.url:
                print(f"[SP] Redirecionado para {page.url}. Forçando retorno para DA...")
                await page.goto(URL_DA_SP, wait_until="domcontentloaded", timeout=45000)
            ...
            print(f"[SP] Campanha Detectada: {current_campaign}")
            ...
            await select_dropdown_option_forced(page, '//*[@id="Discador"]/div[1]/div/div/div/div[2]/div[1]/div[2]/div/div/button', current_campaign, "SP")
            await select_dropdown_option_forced(page, '//*[@id="Discador"]/div[1]/div/div/div/div[2]/div[1]/div[3]/div/div/button', current_campaign, "SP", is_telefone=True)
            await select_dropdown_option_forced(page, '//*[@id="Discador"]/div[1]/div/div/div/div[2]/div[1]/div[6]/div/div[1]/button', fila_name, "SP")

            # Ação de agressividade removida
            
            print(f"[SP] Disparando com {SAIDAS_VALOR} canais...")
            await page.locator("#saida").fill(SAIDAS_VALOR)
            await page.wait_for_timeout(1000)
            await page.locator("#btCampanha1").dispatch_event("click")

            await page.wait_for_timeout(8000)
            print("[SP] ✅ RESTART CONCLUÍDO COM SUCESSO.")
            return True

        except Exception as e:
            if "Timeout" in str(e) and "btCampanha1" in str(e):
                print("[SP] ✅ Sucesso detectado (Timeout ignorado após ação).")
                return True
            print(f"❌ Erro Railway SP: {e}")
            return False
        finally:
            await browser.close()

if __name__ == '__main__':
    # Força SP independente da variável de ambiente
    asyncio.run(restart_campaign(server="SP"))
